...
function createLi(text) {
  ...
  //width >= height
  if (PageWithHeightRatio() >= changeRatio) {
    return `<li onmouseover="Highlight(this)" onmouseout="NotHighLight(this)" style="padding-top:3%;font-size:3em;cursor:pointer" onclick="goToUrl('?${text.replace(
      / /g,
      "-"
    )}')">${text}</li>`;
  }
  //height > width
    ...
    return `<li onmouseover="Highlight(this)" onmouseout="NotHighLight(this)" style="padding-top:15%;font-size:2em;cursor:pointer" onclick="goToUrl('?${text.replace(
      / /g,
      "-"
    )}')">${text}</li>`;
  }
